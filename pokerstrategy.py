...
class StrategyProfile(object):
    ...
    def expected_value(self):
        ...
        for ev in expected_values:
            assert(len(ev) == 1)
        return tuple((iter(list(ev.values())).__next__()) for ev in expected_values) # pull the EV from the dict returned
    ...
    def best_response(self):
        ...
        for ev in expected_values:
            assert(len(ev) == 1)
        expected_values = tuple(iter(list(ev.values())).__next__() for ev in expected_values) # pull the EV from the dict returned
